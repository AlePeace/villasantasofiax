...
export const MainMenu = ({
  ...
  useGSAP(() => {
    ...
    if (logoRef.current) gsap.set(logoRef.current, { opacity: 0, y: 20 });
  ...
  return (
    <>
            ...
            >
              <span
                className={`block h-px transition-all duration-400 origin-center ${
                  isOpen ? "bg-blue rotate-45 translate-y-[8.5px]" : "bg-white"
                }`}
              />
              <span
                className={`block h-px transition-all duration-300 ${
                  isOpen ? "bg-blue opacity-0 scale-x-0" : "bg-white"
                }`}
              />
              <span
                className={`block h-px transition-all duration-400 origin-center ${
          ...
          <div className="flex justify-end">
            <Link
              ...
              className={`font-montecatini uppercase lg:text-xl text-sm tracking-[0.18em] transition-all duration-500 ${
                isOpen ? "text-blue hover:text-lightblue" : "text-white hover:text-blue"
              }`}
            >
              Prenota ora
            </Link>
      ...
      <div
        ref={overlayRef}
        className={`fixed inset-0 z-[9998] opacity-0 bg-blue flex flex-col items-center justify-between px-8 lg:px-20 pt-28 lg:pt-36 pb-10 lg:pb-16 overflow-y-auto ${
      ...
      >
        ...
        {bgImage?.sourceUrl && (
          <div className="absolute inset-0 w-full h-full overflow-hidden">
            ...
            />
            <div className="absolute z-10 inset-0 bg-overlay/85"></div>
          </div>
        )}
        {/* Menu items */}
        <nav ref={menuNavRef} >
          {nodes.map((item, idx) => (
            <Link
              ...
              className="block relative font-montecatini text-4xl lg:text-6xl xl:text-7xl text-blue uppercase tracking-wide leading-none py-3 lg:py-4 border-b border-white/10 hover:text-lightblue transition-colors duration-300 group z-20"
            >
        ...
        {/* Footer of overlay */}
        <div className="self-start relative main-menu__footer flex flex-col lg:flex-row items-start lg:items-end justify-between gap-6 mt-12 opacity-0 translate-y-5 z-20">
          <div className="flex flex-col gap-1">
            <p className="font-montecatini font-light text-blue text-sm lg:text-2xl uppercase tracking-[0.2em]">
              socials
            </p>
          </div>
        </div>
